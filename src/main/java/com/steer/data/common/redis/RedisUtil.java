...
//         int result = 0;
//         try {
//             //连接本地Redis服务    因为我本地的缓存是没有密码的，所以可以达到目的的。
...
//    }
    ...
    public  Boolean getRedisIsOk() {
        log.info("Redis服务心跳检测...");
         try {
             redisTemplate.opsForValue().set("REDIS_STAUTS", "OPEN");
             return true;
         } catch (Exception e) {
             log.error("redis不存活，关闭缓存！");
             return false;
         }
    }
    ...
    public String getKeyForAop(JoinPoint joinPoint, HttpServletRequest request){
        ...
        if(objects==null || objects.length<=0){
            args= Lists.newArrayList(joinPoint.getArgs()).toString();
        }else{
            args = SerializeUtil.serializeObject(objects[0]);
        }
     ...
     * @return true成功 false 失败
     */
    public boolean setByMill(String key, Object value, long time) {
        try {
            if (time > 0) {
